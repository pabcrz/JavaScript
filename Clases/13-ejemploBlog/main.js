...
const createBlogCard = (entryObject) => {
  ...
  let entryTitle = document.createTextNode(autor)

  cardBody.append(cardTitle, cardText)
  card.append(cardImage, cardBody)
  entryName.append(entryTitle)
  itemAnchor.append(entryName)
  autorItem.append(autorImg, itemAnchor)
  cardBody.append(autorItem)
...
const createPopularItem = (entryObject) => {
  ...
  entryItem.classList.add('list-group-item', 'p-2')
...
printPopularEntries(
  blogEntries.filter((entry) => entry.rating > 8),
  'popular-entries'
)
...
const createAutorItem = (entryObject) => {
  ...
  let itemAnchor = document.createElement('a')
  itemAnchor.setAttribute('href', '#')
  itemAnchor.classList.add('link-underline-opacity-0')
...
const printAutors = (autorsArray, wrapperId) => {
  console.log(autorsArray)
  let wrapper = document.getElementById(wrapperId)
  autorsArray.forEach((autor) => {
    let autorEntry = createAutorItem(autor)
    wrapper.append(autorEntry)
  })
}
